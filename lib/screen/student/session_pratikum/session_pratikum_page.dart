...
class SessionPratikumPage extends StatefulWidget {
  const SessionPratikumPage({super.key});

  @override
  State<SessionPratikumPage> createState() => _SessionPratikumPageState();
}

class _SessionPratikumPageState extends State<SessionPratikumPage> {
  String nameSubject = '';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      var viewModel =
          Provider.of<SessionPratikumViewModel>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Consumer<SessionPratikumViewModel>(
                        ...
                        onTap: () {
                          if (value.session.statusSession[index] == 'hadir' ||
                            ...
                            value.presence(
                                ...
                                int.parse(value.session.sessions[index]
                                    .classroomspratikumId),
                                value.session.sessions[index].date,
                                value.session.sessions[index].finish,
                                {
                                  'lecturer': value
                                      .session.sessions[index].student.name,
                                  'session_id':
                                      value.session.sessions[index].id,
                                  'classroomsId': value.session.sessions[index]
                                      .classroomspratikumId,
